...
export default function RootLayout({
        ...
        <main>
          <Context>
            <nav style={{ height: "50px", position:'fixed'}} className="bg-gray-800 dark:bg-gray-900 font-bold fixed w-full top-0 left-0 border-b border-gray-200 dark:border-gray-600 flex items-center nav z-10">
              ...
              <HomeLink />
              <Link className="sm:ml-2 md:ml-6 text-sm sm:text-base md:text-lg lg:text-xl nav-el rg-bt" href="/register">
                Register
              </Link>
              </div>
            </nav>
  ...
  export function HomeLink() {
    ...
    return (
      <>
        <Refresh />
        <Link className="text-sm sm:text-base md:text-lg lg:text-xl nav-el hm-bt" href="/" onClick={handleHomeClick}>
          Home
        </Link>
      </>
    )}